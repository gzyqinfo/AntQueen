...
public class RSAUtil {
    ...
    public byte[] decryptByPrivateKey(String privateKeyStr, byte[] data) throws Exception {
    ...
    public byte[] sign(String privateKeyStr,byte[] data) throws NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        // 实例化Signature
            Signature signature = Signature.getInstance("SHA1withRSA");

            // 初始化Signature
            signature.initSign(this.restorePrivateKey(privateKeyStr));
    ...
    public boolean verify(String publicKeyStr, byte[] data, String sign) throws Exception {
        Signature signature = Signature.getInstance("SHA1withRSA");
        signature.initVerify(restorePublicKey(publicKeyStr));
        signature.update(data);
        return signature.verify(Base64.decodeBase64(sign));
    }
