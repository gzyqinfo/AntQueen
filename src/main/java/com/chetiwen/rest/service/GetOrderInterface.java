...
public class GetOrderInterface {
    ...
    public Response processRequest(ClientRequest clientRequest) throws Exception {
        logger.info("-------------------------------------------------------------------------------------------------------");
        logger.info("Received Get Order request with : {}", clientRequest);
        ...
        try {
            ...
            if (!Authentication.authenticateTime(clientRequest.getTimestamp())) {
                ...
                String sourceOrderNo = clientRequest.getBody().getOrderNo();
                if (OrderMapCache.getInstance().getOrderMap().containsKey(clientRequest.getBody().getOrderNo())) {
                    logger.info("there is cached order");
                    sourceOrderNo = OrderMapCache.getInstance().getByKey(clientRequest.getBody().getOrderNo()).getOrderNo();
            ...
            try {
                ...
                log.setLogType("original getOrder request");
                ...
                if (toSourceRequest != null) {
                    ...
                    requestlog.setLogType("source getOrder request");
                    ...
                    responselog.setLogType("source getOrder response");
